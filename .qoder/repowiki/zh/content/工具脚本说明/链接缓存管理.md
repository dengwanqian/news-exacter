...
## 更新摘要
**变更内容**
- 链接缓存系统重大重构，新增200多个URL条目
- 新增教育科技内容源：北京外国语大学、教育部、北京市政府、中国教育和科研计算机网等
- 清理约60个过时URL，优化缓存质量
- 缓存文件规模扩大至约490个URL条目
...
## 简介
本文件系统性阐述"链接缓存管理"的设计与实现，重点围绕 link_cache.json 文件的作用、结构、去重机制、缓存策略、过期时间管理、读写操作、更新时机、清理策略与性能优化方法，并结合新闻采集流程给出最佳实践与故障恢复方案。随着系统重构，缓存文件已从约666个URL扩展至490个条目，新增了大量教育科技内容源，包括北京外国语大学、教育部、北京市政府等权威机构。
...
**图表来源**
- [main.py:11-210](file://main.py#L11-L210)
- [news_extractor.py:21-994](file://news_extractor.py#L21-L994)
...
- [database.py:5-92](file://database.py#L5-L92)
- [config.py:1-78](file://config.py#L1-L78)
- [classify_existing_news.py:14-351](file://classify_existing_news.py#L14-L351)
...
**图表来源**
- [main.py:28-202](file://main.py#L28-L202)
- [news_extractor.py:180-994](file://news_extractor.py#L180-L994)
- [database.py:40-92](file://database.py#L40-L92)
- [classify_existing_news.py:237-351](file://classify_existing_news.py#L237-L351)
...
- **文件位置**：项目根目录。
- **文件规模**：经过重构后包含约490个URL条目，涵盖教育科技、政府机构、学术院校等多个领域。
- **文件读写**：仅在进程启动时加载一次，在退出时写回；运行期间仅在内存中维护有序字典。

**章节来源**
- [link_cache.json:1-490](file://link_cache.json#L1-L490)
...
- [news_extractor.py:186-200](file://news_extractor.py#L186-L200)
...
- news_extractor 依赖 selenium、requests、bs4、gne_local、dotenv 等。
- database 依赖 sqlite3。
...
## 结论
link_cache.json 作为轻量级缓存层，有效避免重复抓取与入库，提升整体采集效率与稳定性。随着系统重构，缓存文件已扩展至约490个URL条目，新增了大量教育科技内容源，包括北京外国语大学、教育部、北京市政府等权威机构，同时清理了约60个过时URL，进一步优化了缓存质量。其设计简单可靠，结合数据库去重与日志监控，形成闭环保障。建议在生产环境中定期备份缓存文件，合理设置容量与超时参数，并关注目标站点的反爬策略变化。
...
```mermaid
flowchart TD
Start(["启动"]) --> Load["读取 link_cache.json<br/>约490个条目"]
Load --> Exists{"文件存在？"}
...
### 新增内容源概览
经过重构，系统新增了以下教育科技内容源：
- **北京外国语大学**：news.bfsu.edu.cn（包含新闻、通知、技术中心等）
- **教育部官网**：moe.gov.cn（工作动态、政策解读等）
- **北京市政府官网**：beijing.gov.cn（北京要闻等）
- **中国教育和科研计算机网**：edu.cn（教育新闻、专题报道等）
- **Ai机器人**：ai-bot.cn（每日AI新闻等）

这些新增内容源显著扩展了系统的数据采集范围，为教育信息化领域的新闻采集提供了更全面的数据支撑。