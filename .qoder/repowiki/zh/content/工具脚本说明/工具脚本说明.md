...
## 简介
本项目围绕“新闻采集—摘要—分类—展示”的完整流水线，提供多类工具脚本，覆盖数据采集、摘要生成、分类处理、数据库检查与HTML/PDF报告生成。本文档聚焦以下四个工具脚本：
...
- 工具脚本：check_db.py、summary_with_ark.py、classify_existing_news.py
...
- 工具脚本：独立运行的辅助工具，分别用于数据库检查、AI摘要生成、现有新闻分类、链接缓存维护。
...
- 工具脚本（check_db.py、summary_with_ark.py、classify_existing_news.py）各自独立运行
...
## 结论
本项目通过工具脚本与主流程的协同，实现了从新闻采集、摘要、分类到报告生成的闭环。四个核心工具脚本分别承担：
- 数据库健康检查（check_db.py）
