...
- 分类脚本classify_existing_news.py：负责二次分类与最终分类更新。
...
## 架构总览
数据库管理模块采用“单连接单游标”的轻量级设计，初始化即建立连接并创建表；提供插入、查询、更新与关闭方法。主流程在插入前通过标题存在性检查避免重复，分类脚本在后续阶段补充分类信息。
...
Main->>Clf : 启动二次分类
Clf->>DB : 查询无分类/无最终分类新闻
Clf->>Clf : 调用分类API
Clf->>DB : update_category/update_final_category
```
...
- 查询视图：
  - 正常视图：排除final_category为“待审”的记录，按publish_time降序
...
- 查询（正常视图）：SELECT * FROM news WHERE final_category != '待审' ORDER BY publish_time DESC [LIMIT ...]
...
- 查询优化建议
  ...
  - 对final_category建立索引以优化“排除待审”的过滤。
- 批量策略
  - 当前实现逐条插入与更新，适合小规模数据。
...
- 模块间耦合
  ...
  - classify_existing_news.py依赖database.py进行分类更新
...
## 结论
NewsDatabase类以简洁可靠的单连接设计满足了新闻数据的存储需求，结合唯一约束与存在性检查有效避免重复，配合主流程与分类脚本形成完整的数据生命周期管理。建议在生产环境中引入索引、批处理与连接池，进一步提升性能与稳定性。
